  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      <div className="space-y-2">
        ...
        <Input id="login-email" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="почта@example.com" required />
      </div>
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-4">
          <label className="text-sm font-medium text-neutral-800" htmlFor="login-password">Пароль</label>
          <Link className="text-sm font-semibold text-neutral-700 hover:text-neutral-950 hover:underline" href="/forgot-password">Забыли пароль?</Link>
        </div>
...
export function RegisterForm() {
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      <div className="space-y-2"><label className="text-sm font-medium text-neutral-800" htmlFor="register-email">Email</label><Input id="register-email" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="почта@example.com" required /></div>
...
export function ForgotPasswordForm() {
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      <div className="space-y-2">
        <label className="text-sm font-medium text-neutral-800" htmlFor="forgot-email">Email</label>
        <Input id="forgot-email" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="почта@example.com" required />
      </div>
      {error ? <p role="alert" className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p> : null}
      {success ? <p role="status" className="rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-700">Если аккаунт с таким email существует, мы отправили письмо со ссылкой для смены пароля.</p> : null}
      <Button className="w-full" type="submit" disabled={loading}>{loading ? "Отправляем письмо…" : "Отправить ссылку"}</Button>
      <p className="text-center text-sm text-neutral-500"><Link className="font-semibold text-neutral-950 hover:underline" href="/login">Вернуться ко входу</Link></p>
    </form>
  );
}
...
export function ResetPasswordForm() {
  ...
  if (success) {
    return (
      <div className="space-y-5">
        <p role="status" className="rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-700">Пароль успешно изменён. Теперь можешь войти с новым паролем.</p>
        <Button className="w-full" type="button" onClick={() => window.location.assign("/dashboard")}>Перейти в личный кабинет</Button>
      </div>
    );
  }
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      <div className="space-y-2"><label className="text-sm font-medium text-neutral-800" htmlFor="reset-password">Новый пароль</label><Input id="reset-password" type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Не менее 6 символов" required minLength={6} /></div>
      <div className="space-y-2"><label className="text-sm font-medium text-neutral-800" htmlFor="reset-confirmation">Повтори новый пароль</label><Input id="reset-confirmation" type="password" autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder="Повтори пароль" required minLength={6} /></div>
      {error ? <p role="alert" className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p> : null}
      <Button className="w-full" type="submit" disabled={loading}>{loading ? "Сохраняем пароль…" : "Сохранить новый пароль"}</Button>
    </form>
  );
}
...
export function LogoutButton() {
  ...
  return <Button variant="outline" size="sm" type="button" onClick={handleLogout} disabled={loading}>{loading ? "Выходим…" : "Выйти"}</Button>;
}
