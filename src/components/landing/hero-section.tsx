...
> = {
  urgent: {
    ...
    description: "Приоритет на максимальную скорость доставки",
    ...
    product: {
      ...
      reasons: ["Сборка за 45 секунд", "Доставка со склада за 4 часа", "Влагозащита дна 5000 мм"],
      antiFake: "92% реальных отзывов",
    },
  },
  perfectionist: {
    label: "Перфекционист",
    ...
    product: {
      ...
      reasons: [
        "Алюминиевый каркас",
        "Двухслойная мембрана RipStop",
        "0% жалоб на брак за 6 месяцев",
      ],
      antiFake: "98% реальных покупателей",
    },
  },
  budget: {
    label: "Экономный",
    ...
    description: "Приоритет на минимальную цену и честные скидки",
  ...
  parents: {
    label: "Мама / Папа",
    emoji: "👶",
    placeholder: "Безопасная детская палатка-домик...",
    description: "Приоритет на безопасность и надежность материалов",
    ...
    product: {
      ...
      platform: "Wildberries",
      delivery: "Завтра",
      reasons: ["Натуральный хлопок", "Безопасные опоры из бука", "Плотный чехол для хранения"],
      antiFake: "Проверено ИИ: реальные отзывы",
    },
  },
};
...
export default function HeroSection() {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-[#0D0F14] px-4 pb-4 pt-16 text-slate-100 selection:bg-[#00FF87] selection:text-black md:px-8 md:pt-18 lg:px-16">
      ...
      <main className="relative z-10 mx-auto grid min-h-[calc(100svh-64px)] w-full max-w-7xl grid-cols-1 items-center gap-5 py-3 lg:grid-cols-12 lg:gap-6">
        <div className="col-span-1 flex flex-col justify-center space-y-4 lg:col-span-7">
          ...
          <p className="max-w-xl text-base leading-relaxed text-slate-400 md:text-lg">
            wobuy. анализирует предложения, отсекает фейковые отзывы, рассчитывает честный рейтинг и
            выдает лучшие варианты с человеческим объяснением.
          </p>
          <form onSubmit={submitSearch} className="group relative w-full max-w-2xl">
            ...
            <div className="relative flex items-center rounded-2xl border border-white/10 bg-[#13161C]/90 p-2 pl-4 transition-all focus-within:border-[#00FF87]/50">
              ...
              <input
                ...
                className="w-full border-none bg-transparent pr-4 text-sm text-white outline-none placeholder:text-slate-500 md:text-base"
                aria-label="Поиск товаров"
              />
              <button
                type="submit"
                className="flex shrink-0 items-center justify-center rounded-xl bg-[#00FF87] px-6 py-3 font-bold text-black shadow-lg shadow-emerald-500/10 transition-all hover:bg-[#00E576]"
              >
                <span className="mr-2 hidden md:inline">Найти лучшее</span>
              ...
              {(Object.keys(ARCHETYPES) as Archetype[]).map((key) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-semibold transition-all md:text-sm ${selected ? "border border-white/20 bg-white/10 text-white" : "border border-transparent bg-white/5 text-slate-400 hover:bg-white/10 hover:text-white"}`}
                  >
        ...
        <div className="relative col-span-1 flex min-h-[350px] items-center justify-center lg:col-span-5">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="relative w-full max-w-[380px] rounded-3xl border border-white/10 bg-[#13161C]/60 p-6 shadow-2xl backdrop-blur-xl"
            >
                ...
                <div className="relative flex h-12 w-12 items-center justify-center">
                  <svg className="h-full w-full -rotate-90">
                    ...
                    <circle
                      ...
                      fill="transparent"
                      strokeDasharray="125"
                      strokeDashoffset={125 - (125 * config.product.score) / 10}
                    />
                  </svg>
      ...
      <section
        id="features"
        className="relative z-10 mx-auto max-w-7xl scroll-mt-20 border-t border-white/5 py-6"
      >
        <div className="grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            <Sparkles className="mb-3 h-6 w-6 text-[#00FF87]" />
            <h2 className="text-lg font-bold text-white">Собираем предложения</h2>
            <p className="mt-2 text-sm leading-relaxed text-slate-400">
              Сейчас это демо-каталог с эмуляцией Ozon, Wildberries и Яндекс Маркета.
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            <ShieldCheck className="mb-3 h-6 w-6 text-[#00FF87]" />
            <h2 className="text-lg font-bold text-white">Проверяем качество</h2>
            <p className="mt-2 text-sm leading-relaxed text-slate-400">
              Показываем рейтинг, отзывы, доставку и аргументы, чтобы сравнение было понятным.
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            <Truck className="mb-3 h-6 w-6 text-[#00FF87]" />
            <h2 className="text-lg font-bold text-white">Показываем лучшее</h2>
            <p className="mt-2 text-sm leading-relaxed text-slate-400">
              Переходи в карточку товара, сравнивай предложения и открывай нужное предложение.
            </p>
          </div>
        </div>
      </section>
      <section
        ...
        className="relative z-10 mx-auto max-w-7xl scroll-mt-20 border-t border-white/5 py-6"
      >
        <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 text-center md:p-8">
          <span className="text-xs font-bold uppercase tracking-wider text-[#00FF87]">Тарифы</span>
          <h2 className="mt-2 text-3xl font-extrabold text-white">На этапе демо — бесплатно</h2>
          <p className="mx-auto mt-2 max-w-2xl text-sm leading-relaxed text-slate-400">
            Сначала доводим пользовательский сценарий и механику до рабочего состояния. Монетизацию
            подключим после проверки продукта.
          </p>
        </div>
      </section>
      <section
      ...
      >
        <div className="max-w-2xl">
          <span className="text-xs font-bold uppercase tracking-wider text-[#00FF87]">О нас</span>
          <h2 className="mt-2 text-3xl font-extrabold text-white">
            wobuy. — сервис осознанного выбора
          </h2>
          <p className="mt-3 text-sm leading-relaxed text-slate-400">
            Мы строим интерфейс, в котором сложный анализ покупок превращается в понятную
            рекомендацию. Сейчас продукт работает в демо-режиме на подготовленных данных.
          </p>
        </div>
      </section>
      <footer className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-2 border-t border-white/5 pb-1 pt-4 text-xs text-slate-500 md:flex-row md:items-center md:justify-between">
        <div>© 2026 wobuy. — Умный ИИ-помощник для покупок.</div>
