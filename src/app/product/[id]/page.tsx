    ...
    <main className="min-h-screen bg-[#0D0F14] px-4 py-6 text-slate-100 md:px-8 md:py-10">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex items-center justify-between gap-4">
          <Link href="/search" className="text-sm font-semibold text-slate-400 hover:text-white">
            ← Назад к поиску
          </Link>
          <Link href="/dashboard" className="text-sm font-semibold text-[#00FF87] hover:underline">
            Личный кабинет
          </Link>
        </div>
        <section className="grid overflow-hidden rounded-3xl border border-white/10 bg-[#13161C]/70 backdrop-blur-xl md:grid-cols-2">
          ...
          <div className="p-6 md:p-10">
            <div className="mb-2 text-xs font-bold uppercase tracking-wider text-[#00FF87]">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {user ? (
                <form action={toggleFavorite.bind(null, product.id)}>
                  <button
                    type="submit"
                    className={`flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-bold ${favorite ? "bg-[#00FF87] text-black" : "border border-white/10 bg-white/5 text-white hover:bg-white/10"}`}
                  >
                    <Heart className="h-4 w-4" fill={favorite ? "currentColor" : "none"} />
                    {favorite ? "В избранном" : "Сохранить товар"}
                  </button>
                </form>
              ) : (
                <Link
                  href="/login"
                  className="rounded-xl bg-[#00FF87] px-4 py-3 text-sm font-bold text-black"
                >
                  Войти, чтобы сохранить
                </Link>
              )}
            </div>
            <div className="mt-6 flex items-center gap-2 rounded-xl border border-emerald-500/10 bg-emerald-500/5 px-4 py-3 text-sm text-slate-300">
              <ShieldCheck className="h-5 w-5 text-[#00FF87]" />
              Демонстрационный каталог для полной отладки пользовательского сценария
            </div>
            {product.ai_summary ? (
        ...
        <section className="mt-8">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500">{offers.length} варианта</span>
          </div>
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {offers.map((offer) => (
              ...
              >
                ...
                <h3 className="mt-4 line-clamp-2 text-sm font-bold text-white">{offer.title}</h3>
                <div className="mt-4 text-2xl font-black text-[#00FF87]">
                  {formatPrice(offer.price, offer.currency)}
                </div>
                <div className="mt-2 flex items-center gap-2 text-xs text-slate-500">
                  <Truck className="h-3.5 w-3.5" />
                  {offer.delivery_text || "Уточняется"}
                </div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="mt-5 flex items-center justify-center gap-2 rounded-xl bg-[#00FF87] py-3 text-xs font-extrabold text-black hover:bg-[#00E576]"
                >
