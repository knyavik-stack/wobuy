...
export default async function SearchResults({ query, products, categories, category, sort, view }: { query: string; products: SearchProduct[]; categories: string[]; category: string; sort: string; view: "grid" | "list" }) {
  ...
  const sortLabels: Record<string, string> = { relevance: "По названию", price_asc: "Сначала дешевле", price_desc: "Сначала дороже", rating: "По рейтингу" };
      ...
      <section className="mb-4 rounded-2xl border border-white/10 bg-[#13161C]/80 p-2.5 shadow-xl shadow-black/10">
        <details className="group">
          <summary className="flex cursor-pointer list-none items-center justify-between rounded-xl px-3 py-2.5 transition hover:bg-white/[0.04] [&::-webkit-details-marker]:hidden">
            <div className="flex min-w-0 items-center gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#00FF87]/10 text-[#00FF87]"><SlidersHorizontal className="h-4 w-4" /></span><div className="min-w-0"><div className="text-sm font-bold text-white">Фильтры</div><div className="truncate text-[11px] text-slate-500">{activeCategory} · {activeSort}</div></div></div>
            <div className="flex shrink-0 items-center gap-2"><span className="hidden text-[11px] font-semibold text-slate-500 sm:inline">Настроить</span><ChevronDown className="h-4 w-4 text-slate-500 transition-transform group-open:rotate-180" /></div>
          </summary>
          <div className="mt-2 border-t border-white/5 px-3 pb-2 pt-4">
            <div className="grid gap-5 md:grid-cols-[1fr_auto]">
              <div><div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-600">Категория</div><div className="flex flex-wrap gap-2"><Link href={href(query,"all",sort,view)} className={`${filterBase} ${category === "all" ? "border-[#00FF87]/30 bg-[#00FF87]/10 text-[#00FF87]" : ""}`}>Все</Link>{categories.map((c) => <Link key={c} href={href(query,c,sort,view)} className={`${filterBase} ${category === c ? "border-[#00FF87]/30 bg-[#00FF87]/10 text-[#00FF87]" : ""}`}>{c}</Link>)}</div></div>
              <div className="md:min-w-[330px]"><div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-600">Сортировка</div><div className="flex flex-wrap gap-2">{[["relevance","По названию"],["price_asc","Дешевле"],["price_desc","Дороже"],["rating","Рейтинг"]].map(([s,label]) => <Link key={s} href={href(query,category,s,view)} className={`${filterBase} ${sort === s ? "border-[#00FF87]/30 bg-white/10 text-white" : ""}`}>{label}</Link>)}</div></div>
            </div>
          </div>
        </details>
      </section>
